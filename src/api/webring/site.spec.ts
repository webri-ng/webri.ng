...
describe('Get new site API', function () {
	...
	let testWebringWithExtraUrlComponents: Webring;
	...
	before(async function beforeTesting() {
		...
		testWebringWithExtraUrlComponents = await testUtils.insertTestWebring(
			testUser.userId!
		);
		...
		testSite6 = await testUtils.insertTestSite(
			testWebringWithExtraUrlComponents.ringId!,
			testUser.userId!,
			{
				dateCreated: dayjs().subtract(2, 'days').toDate(),
		...
		testSite7 = await testUtils.insertTestSite(
			testWebringWithExtraUrlComponents.ringId!,
			testUser.userId!,
			{
				dateCreated: dayjs().subtract(1, 'days').toDate(),
		...
		testSite8 = await testUtils.insertTestSite(
			testWebringWithExtraUrlComponents.ringId!,
			testUser.userId!,
			{
	...
	it('should return a 404 status when passed a webring with no sites added', function (done) {
		chai
			.request(app)
	...
	it('should return a 404 status when passed a valid webring, and an invalid method', function (done) {
		chai
			.request(app)
	...
	it('should redirect to the base domain URL when passed a nonexistent webring url', function (done) {
		chai
			.request(app)
			.get('/webring/@@@@@/random')
			.redirects(0)
	...
	it('should redirect to the first site when not passed an index and the method is next', function (done) {
		chai
			.request(app)
	...
	it('should redirect to the first site when not passed an index and the method is previous', function (done) {
		chai
			.request(app)
			.get(`/webring/${testWebring?.url}/next`)
			.redirects(0)
	...
	it('should redirect to the first site when passed a negative index and the method is next', function (done) {
		chai
			.request(app)
	...
	it('should redirect to the first site when passed a negative index and the method is previous', function (done) {
		chai
			.request(app)
	...
	it('should redirect to the first site when passed an invalid index', function (done) {
		chai
			.request(app)
	...
	it('should redirect to the first site when passed an index that is too high', function (done) {
		chai
			.request(app)
	...
	it("should redirect to the first site when the method is 'next' and passed the last index", function (done) {
		chai
			.request(app)
	...
	it("should return the last site when the method is 'previous' and passed the first index", function (done) {
		chai
			.request(app)
	...
	it('should return a random site', function (done) {
		chai
			...
			.end(function (err, res) {
				expect(err).to.be.null;
				expect(res.status).to.equal(303);
				expect(res.header.location).to.be.oneOf([
					testSite.url,
					testSite2.url,
					testSite3.url,
					testSite4.url,
	...
	it('should handle being passed multiple referring urls', function (done) {
		chai
			.request(app)
			.get(
				`/webring/${testWebring?.url}/previous?via=${testSite2.url}&via=${testSite3.url}`
			)
			.redirects(0)
			.end(function (err, res) {
				expect(err).to.be.null;
	...
	it("should handle being passed a referring url with a 'via' query parameter", function (done) {
	...
	it("should redirect to the first site when the method is 'next' and passed the last url", function (done) {
		chai
			.request(app)
	...
	it("should return the last site when the method is 'previous' and passed the first url", function (done) {
		chai
			.request(app)
	...
	it('should handle being passed a URL with an encoded anchor', function (done) {
		const encodedUrl = encodeURIComponent(testSite6.url);

		chai
			.request(app)
			.get(
				`/webring/${testWebringWithExtraUrlComponents?.url}/next?via=${encodedUrl}`
			)
			.redirects(0)
			.end(function (err, res) {
				expect(err).to.be.null;
	...
	it('should handle being passed a URL with an encoded query parameter', function (done) {
	...
	it('should redirect to the next site when passed a valid encoded url', function (done) {
		const encodedUrl = encodeURIComponent(testSite2.url);

		chai
			.request(app)
			.get(`/webring/${testWebring?.url}/next?via=${encodedUrl}`)
			.redirects(0)
			.end(function (err, res) {
				expect(err).to.be.null;
				expect(res.status).to.equal(303);
				expect(res.header.location).to.equal(testSite3.url);
				done();
			});
	});
});
