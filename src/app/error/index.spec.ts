...
describe('API Returnable Error Instance', function () {
	it('should be able to instantiate an instance from error detail instance', function () {
		const errorInstance = ApiReturnableError.fromApiErrorResponseDetails(
		...
		expect(errorInstance.httpStatus).to.equal(
			requestValidationError.httpStatus
		);
	});

	it('the HTTP status code should default to 400 when not specified', function () {
