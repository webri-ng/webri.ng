...
describe('Add site to webring', function () {
	...
	it('should add a site to a webring', async function () {
		...
		const testSite = await addNewSite(testWebring, name, url, testUser.userId!);
	...
	it(
		'should raise an exception if a site with the same URL already exists ' +
			'in this webring',
		async function () {
			const name = createRandomString();
			const url = testUtils.createRandomSiteUrl();

			const testSite = await addNewSite(
				testWebring,
				name,
				url,
				testUser.userId!
