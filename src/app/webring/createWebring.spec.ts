...
describe('Create new webring', function () {
	...
	it('should raise an exception if more tags than the maximum allowed number is specified', async function () {
		const name = createRandomString();
		const url = testUtils.createRandomWebringUrl();
		const tags = Array(webringConfig.maxTagCount + 1)
			.fill('')
			.map(() => createRandomString());
	...
	it('should create a webring', async function () {
		...
		testWebring = await createWebring(
			name,
			url,
			'description',
			false,
		...
		expect(dayjs(testWebring.dateCreated).isSame(new Date(), 'hour')).to.be
			.true;
