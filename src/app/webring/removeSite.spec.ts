...
describe('Remove webring site', function () {
	...
	it('should raise an exception if a non-serialised webring is provided', async function () {
		return expect(
			removeSite(nonSerialisedWebring, testSite3.url)
	...
	it('should throw an exception when passed a nonexistent url', async function () {
		return expect(
			removeSite(testWebring, testUtils.dummyUuid)
		).to.be.rejectedWith(ApiReturnableError, siteNotFoundError.message);
	});

	it('should throw an exception when passed an empty url', async function () {
		return expect(removeSite(testWebring, '')).to.be.rejectedWith(
			ApiReturnableError,
			siteNotFoundError.message
		);
	});

	it('should throw an exception when passed a site from another webring', async function () {
		return expect(removeSite(testWebring, testSite3.url)).to.be.rejectedWith(
			ApiReturnableError,
			siteNotFoundError.message
		);
	});
	...
	it('should throw an exception when passed a deleted site', async function () {
		return expect(removeSite(testWebring2, testSite4.url)).to.be.rejectedWith(
			ApiReturnableError,
			siteNotFoundError.message
		);
	});
	...
	it('should correctly remove a site from a webring', async function () {
		...
		const testWebringSites = await webringService.getWebringSites(
			testWebring.ringId!
		);
