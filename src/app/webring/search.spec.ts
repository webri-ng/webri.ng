...
describe('Search Webrings', function () {
	...
	describe('Search webring by tag', function () {
		it('should correctly raise an exception if an invalid tag name is provided', async function () {
			return expect(search(SearchWebringsMethod.Tag, '')).to.be.rejectedWith(
				ApiReturnableError,
				invalidTagErrorMessage
			);
		});
		...
		it('should correctly return an empty array if the specified tag does not exist', async function () {
			const results = await search(
				SearchWebringsMethod.Tag,
				'fffffffffffffffff'
			);
		...
		it('should correctly return an empty array if no webrings are tagged with this tag', async function () {
		...
		it('should correctly return webrings tagged with a certain tag', async function () {
			...
			expect(
				results.webrings.find((ring) => ring.ringId === testWebring.ringId)
			).to.not.be.undefined;
			expect(
				results.webrings.find((ring) => ring.ringId === testWebring2.ringId)
			).to.not.be.undefined;
			...
			expect(
				results.webrings.find((ring) => ring.ringId === testWebring3.ringId)
			).to.not.be.undefined;
			expect(
				results.webrings.find((ring) => ring.ringId === testWebring4.ringId)
			).to.not.be.undefined;
		});
	...
	describe('Search webring by creator', function () {
		it('should correctly raise an exception if no user id is provided', async function () {
			return expect(
				search(SearchWebringsMethod.Creator, '')
			).to.be.rejectedWith(ApiReturnableError, invalidUserIdErrorMessage);
		});

		it('should correctly raise an exception if an invalid user id is provided', async function () {
			return expect(
				search(SearchWebringsMethod.Creator, testUtils.invalidUuid)
			).to.be.rejectedWith(ApiReturnableError, invalidUserIdErrorMessage);
		});
		...
		it('should correctly return an empty array if the specified user has created no webrings', async function () {
			const results = await search(
				SearchWebringsMethod.Creator,
				testUser3.userId!
			);
	...
	describe('Search webring by name', function () {
		it('should correctly raise an exception if no name is provided', async function () {
			return expect(search(SearchWebringsMethod.Name, '')).to.be.rejectedWith(
				ApiReturnableError,
				invalidWebringNameErrorMessage
			);
		});
	...
	describe('Pagination', function () {
		...
		const totalPublicWebrings =
			siteConfig.defaultPageLength * (totalPublicPages - 1) + 3;
		...
		it('should correctly paginate results when searching by tag', async function () {
		...
		it('should correctly paginate results when searching by tag and return private webrings', async function () {
			let results = await search(
				SearchWebringsMethod.Tag,
				testTag6.name || '',
				{
	...
	describe('Sorting', function () {
		...
		it('should correctly sort results in alphabetical order', async function () {
			...
			let previousName = '';
			for (const webring of results.webrings) {
				const comparisonResult = webring.name.localeCompare(
					previousName,
					'en',
					{
