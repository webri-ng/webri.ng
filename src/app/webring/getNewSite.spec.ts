...
describe('Get new site', function () {
	...
	before(async function beforeTesting() {
		...
		testWebringWithOneSite = await testUtils.insertTestWebring(
			testUser.userId!
		);
		testWebringWithTwoSites = await testUtils.insertTestWebring(
			testUser.userId!
		);
		testEmptyWebring = await testUtils.insertTestWebring(testUser.userId!);

		nonSerialisedWebring = new Webring(
			createRandomString(),
			createRandomString(),
			createRandomWebringUrl(),
			false,
			testUser.userId!
		);
		...
		testSite7 = await testUtils.insertTestSite(
			testWebringWithTwoSites.ringId!,
			testUser.userId!,
			{
				dateCreated: dayjs().subtract(2, 'days').toDate()
			}
	...
	it('should raise an exception if a non-serialised webring is provided', async function () {
		return expect(
			getNewSite(nonSerialisedWebring, GetNewSiteMethod.Next)
	...
	it('should return the first site when not passed an index and the method is next', async function () {
		const nextSite = await getNewSite(testWebring, GetNewSiteMethod.Next);
		expect(nextSite!.siteId).to.equal(testSite.siteId);
	});

	it('should return the first site if the webring only has a single site', async function () {
		const nextSite = await getNewSite(
			testWebringWithOneSite,
			GetNewSiteMethod.Next
		);
		expect(nextSite!.siteId).to.equal(testSite6.siteId);
	});

	describe('When passed a site index', function () {
		it('should return the first site when not passed an index and the method is previous', async function () {
		...
		it("should return the first site when the method is 'next' and passed the last index", async function () {
		...
		it('should return the previous site when passed a valid index', async function () {
			const nextSite = await getNewSite(
				testWebring,
				GetNewSiteMethod.Previous,
				3
			);
			expect(nextSite!.siteId).to.equal(testSite3.siteId);
		});

		it("should return the last site when the method is 'previous' and passed the first index", async function () {
			const nextSite = await getNewSite(
				testWebring,
				GetNewSiteMethod.Previous,
				0
	...
	describe('When passed a site URL', function () {
		...
		it("should return the first site when the method is 'next' and passed the last URL", async function () {
			const nextSite = await getNewSite(
				testWebring,
				GetNewSiteMethod.Next,
				undefined,
		...
		it('should return the first site when passed a nonexistent URL', async function () {
			const nextSite = await getNewSite(
				testWebring,
				GetNewSiteMethod.Next,
				undefined,
		...
		it('should return the correct site when passed a valid URL without the protocol', async function () {
		...
		it("should return the last site when the method is 'previous' and passed the first URL", async function () {
			const nextSite = await getNewSite(
				testWebring,
				GetNewSiteMethod.Previous,
				undefined,
		...
		it('should override the index when passed a URL', async function () {
			const nextSite = await getNewSite(
				testWebring,
				GetNewSiteMethod.Next,
				3,
	...
	describe('Get a random site', function () {
		it('should return a different site to the current', async function () {
			let nextSite = await getNewSite(
				testWebringWithTwoSites,
				GetNewSiteMethod.Random,
				0
			);
			expect(nextSite?.siteId).to.equal(testSite8.siteId);

			nextSite = await getNewSite(
				testWebringWithTwoSites,
				GetNewSiteMethod.Random,
				1
			);
