...
describe('Update webring', function () {
	...
	it('should raise an exception if more tags than the maximum allowed number is specified', async function () {
		const name = createRandomString();
		const url = testUtils.createRandomWebringUrl();
		const tags = Array(webringConfig.maxTagCount + 1).map(() =>
			createRandomString()
		);
	...
	it('should update a webring', async function () {
		...
		testWebring = await updateWebring(
			testWebring.ringId!,
			testUser.userId || '',
			name,
			url,
		...
		expect(dayjs(testWebring.dateCreated).isSame(testWebring.dateCreated)).to.be
			.true;
		expect(dayjs(testWebring.dateModified).isSame(new Date(), 'hour')).to.be
			.true;
	...
	it('should update a webring to add new tags', async function () {
		...
		expect(testWebring.tags.find((tag) => tag.name === tag1Text)).to.not.be
			.undefined;
		expect(testWebring.tags.find((tag) => tag.name === tag2Text)).to.not.be
			.undefined;
	});
