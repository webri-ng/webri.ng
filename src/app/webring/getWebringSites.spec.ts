...
describe('Get Webring Sites', function () {
	...
	before(async function beforeTesting() {
		...
		testDeletedSite = await testUtils.insertTestSite(
			testWebring2.ringId!,
			testUser.userId!
		);
	...
	it('should throw an exception when passed an empty ringId', async function () {
		return expect(getWebringSites('')).to.be.rejectedWith(
			ApiReturnableError,
	...
	it('should correctly get the sites for a webring', async function () {
		...
		expect(results.find((site) => site.siteId === testSite.siteId)).to.not.be
			.undefined;
		expect(results.find((site) => site.siteId === testSite2.siteId)).to.not.be
			.undefined;
		expect(results.find((site) => site.siteId === testSite5.siteId)).to.not.be
			.undefined;
	});
	...
	it('should correctly ignore deleted sites', async function () {
		...
		expect(results.find((site) => site.siteId === testSite3.siteId)).to.not.be
			.undefined;
		expect(results.find((site) => site.siteId === testSite4.siteId)).to.not.be
			.undefined;
		expect(results.find((site) => site.siteId === testDeletedSite.siteId)).to.be
			.undefined;
	});
	...
	it("should get a webring's sites within a transaction", async function () {
		await appDataSource.transaction(async (transactionalEntityManager) => {
			...
			expect(results.find((site) => site.siteId === testSite4.siteId)).to.not.be
				.undefined;
			expect(results.find((site) => site.siteId === testDeletedSite.siteId)).to
				.be.undefined;
		});
	});
});
