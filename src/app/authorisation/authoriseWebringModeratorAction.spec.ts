...
describe('Authorise webring moderator action', function () {
	...
	it('should not raise an exception when passed a user that created the webring', async function () {
		return expect(authoriseWebringModeratorAction(testWebring, testUser)).to.not
			.be.rejected;
	});

	it('should not raise an exception when passed a user that is a moderator of the webring', async function () {
		return expect(authoriseWebringModeratorAction(testWebring2, testUser)).to
			.not.be.rejected;
	});
});
