...
describe('User Entity', function () {
	...
	describe('Temporary password expiry date', function () {
		...
		it('should correctly set the temporary expiry date as of the default current date', function () {
	...
	describe('Validate email address', function () {
		it('should throw an exception when an empty email is provided', function () {
			expect(() => User.validateEmailAddress(''))
				.to.throw(ApiReturnableError)
				.and.haveOwnProperty('code', invalidEmailAddressError.code);
		});

		it('should correctly validate a correct email', function () {
			expect(() =>
				User.validateEmailAddress('valid@example.org')
			).to.not.throw();
		});

		it('should correctly validate an incorrect email', function () {
			expect(() => User.validateEmailAddress('invalid@example'))
				.to.throw(ApiReturnableError)
				.and.haveOwnProperty('code', invalidEmailAddressError.code);
			expect(() => User.validateEmailAddress('@example.org'))
				.to.throw(ApiReturnableError)
				.and.haveOwnProperty('code', invalidEmailAddressError.code);
			expect(() => User.validateEmailAddress('a@.org'))
				.to.throw(ApiReturnableError)
				.and.haveOwnProperty('code', invalidEmailAddressError.code);
		});
	});
	...
	describe('Normalise email address', function () {
		it("should correctly normalise a user's email", function () {
			const normalisedEmail: string =
				User.normaliseEmailAddress('  TEST@EXAMPLE.ORG');
	...
	describe('Validate user name', function () {
		...
		it('should throw an exception when passed a username that is too long', function () {
			const longUsername = Array(userConfig.usernameRequirements.maxLength + 1)
				.fill('n')
				.join('');
			expect(() => User.validateUsername(longUsername))
				.to.throw(ApiReturnableError)
				.and.haveOwnProperty('code', invalidUsernameError.code);
		});
